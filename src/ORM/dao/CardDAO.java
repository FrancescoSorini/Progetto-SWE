...
public class CardDAO {
    ...
    public Card getCardById(int id) throws SQLException{
        String sql = " SELECT card_id, card_name, tcg_id FROM cards WHERE card_id = ?";
        try(PreparedStatement stmt = connection.prepareStatement(sql)) {
            stmt.setInt(1, id);
            try(ResultSet rs = stmt.executeQuery()) {
                if (rs.next()) {
                    ...
                    card.setCardType(CardType.values()[rs.getInt("tcg_id")]); // collegamento a CardType
                    return card;
                }
            }
        }
    ...
    public Card getCardByName(String name) throws SQLException {
        String sql = "SELECT card_id, card_name, tcg_id FROM cards WHERE card_name = ?";
        try (PreparedStatement stmt = connection.prepareStatement(sql)) {
    ...
    public List<Card> getAllCards() throws SQLException {
        ...
        String sql = " SELECT card_id, card_name, tcg_id FROM cards";
        try(
    ...
    public void updateCard(Card card) throws SQLException {
        String sql = "UPDATE cards SET card_name = ?, tcg_id = ? WHERE card_id = ?";
        try(PreparedStatement stmt = connection.prepareStatement(sql)) {
            stmt.setString(1, card.getName());
            stmt.setInt(2, card.getType().getId()); // collegamento a CardType
